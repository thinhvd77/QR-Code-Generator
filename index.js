...
const fileFilter = (req, file, cb) => {
    // Chỉ cho phép các loại tệp an toàn (ảnh, pdf, tài liệu office)
...
const uploadLimiter = rateLimit({
    ...
    max: 10, // Giới hạn mỗi IP chỉ được upload 10 file trong 15 phút
    message: {
        error: "Quá nhiều yêu cầu tải tệp, vui lòng thử lại sau 15 phút.",
    },
...
function authorize() {
    try {
        ...
        if (fs.existsSync(TOKEN_PATH)) {
            ...
            throw new Error(
                "Token not found. Run the authentication script to generate token.json."
            );
        }
    } catch (err) {
